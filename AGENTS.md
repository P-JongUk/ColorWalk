...
- Monetization source of truth: `docs/product-growth-strategy.md` under `Monetization Model`. The approved implementation timing and data-preservation procedure live in `docs/post-launch-monetization-and-payment-safety.md`: version 1 launches free, then the first safe post-launch monetization update adds payment as an additive entitlement layer after an actual in-place update preservation check. When the user says `유료 업데이트`, `결제 모듈`, `Hueday Studio`, `Hueday Cloud`, `구매 복원`, `paywall`, or `entitlement`, read and execute that contract before planning or coding. Keep the free core promise and earned badge rewards useful when paid packs or subscriptions are designed.
...
- Daily Color Hunt contract: each device-local calendar day starts with a fresh mission color. Show one weather/time/coarse-location recommendation and allow at most six `다른 색` choices before the first accepted photo. Choices 1–3 reuse the same context; choices 4–6 draw uniformly from the full curated catalog while excluding every color already shown that day. The first accepted photo locks that day's color. Preserve 1–7 photos as a valid daily record, count 8 photos as a completed 3x3 and the major reward unit, close the record at local midnight, and never carry yesterday's color forward as today's active mission.
- First-release privacy baseline: raw/local-master photos remain device-local, cloud upload is preview-only and owner-private, reads use authenticated owner RLS and short-lived signed URLs, and analytics/logs never contain photo data, journal text, signed URLs, tokens, or exact coordinates. Location is foreground-only mission context: do not store or log coordinates/place names, provide a denial fallback, and before release coarsen coordinates sent to the weather provider and verify that uploaded previews contain no EXIF location metadata. Do not add enterprise-scale security infrastructure without a concrete threat or operational need.
...
- First-release representative content is `Living Hue Deck`; source of truth: `docs/living-hue-deck-product-spec.md` and `docs/launch-scope-and-update-safety-contract.md`. `docs/discovered-color-content-strategy.md` keeps the candidate/history map. Hue Canvas is excluded from version 1 but is an approved, mandatory early post-launch feature update; preserve G1 on `feature/hue-canvas-prototype` and implement it only through the versioned migration/update-safety gates in `docs/hue-canvas-product-spec.md`. Hue Room is completely removed from the first-release critical path and deferred as a post-launch hypothesis; preserve its historical spec and local concepts, but do not resume room, furniture, placement, or art-production work without a new explicit user decision and evidence that it is worth the cost.
- Friend features are not first-release scope. `Hue Drop` remains the first social update and its only source of truth is `docs/hue-drop-post-launch-spec.md`: invite-only, authenticated friends, center mission color plus eight optional slots, no public discovery or anonymous UGC. The mandatory Hue Canvas update and the first social update are separate lanes; do not pre-create Hue Drop DB/Realtime skeletons.
...
6. Hue Canvas is a mandatory early post-launch update. Preserve its G1 reference/prototype evidence on D; after version-1 launch and update-safety readiness, continue G2/design/performance work on a fresh branch without merging the prototype branch wholesale.
