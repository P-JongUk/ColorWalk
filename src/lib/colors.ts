...
export function averagePixels(data: Uint8ClampedArray): Rgb {
  let r = 0
  let g = 0
  let b = 0
...
export function detectScreenAbuse(data: Uint8ClampedArray, width: number) {
  const samples = data.length / 4
  const avg = averagePixels(data)
  let variance = 0
  let highContrastRows = 0

  for (let index = 0; index < data.length; index += 4) {
    variance +=
      Math.abs(data[index] - avg.r) +
      Math.abs(data[index + 1] - avg.g) +
      Math.abs(data[index + 2] - avg.b)

    const pixel = index / 4
    if (pixel > width && pixel % width === 0) {
      const prev = index - width * 4
      const rowDelta =
        Math.abs(data[index] - data[prev]) +
        Math.abs(data[index + 1] - data[prev + 1]) +
        Math.abs(data[index + 2] - data[prev + 2])

      if (rowDelta > 90) highContrastRows += 1
    }
  }

  const normalizedVariance = variance / samples
  const tooFlat = normalizedVariance < 6
  const scanlineLike = highContrastRows > width * 0.1
  const tooBright =
    avg.r > 236 && avg.g > 236 && avg.b > 236 && normalizedVariance < 16

  return tooFlat || scanlineLike || tooBright
}
