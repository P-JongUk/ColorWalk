  ...
  return (
    <main className="invite-shell">
      <form className="invite-card" onSubmit={submit}>
        ...
        <p className="text-xs font-black uppercase text-[color:#5f9f91]">{t(locale, 'betaBadge')}</p>
        <h1>Color Walk</h1>
        ...
        {error ? <p className="text-sm font-bold text-[color:#5f9f91]">{t(locale, 'inviteError')}</p> : null}
