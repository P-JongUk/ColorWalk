...
- Cloud는 자동 동기화·기기 복구·고화질 재다운로드라는 저장 가치에만 과금 후보를 둔다. 무료 사용자의 로컬 원본이나 기본 Story export를 잠그지 않는다.
...
### M1 구현 사실 — 2026-07-24 KST

- 사진 0장에는 Post를 만들지 않지만 사용자 ID+현지 날짜의 mission·재추천 횟수는 로컬에 보존한다.
- `이 사진 사용` 확정 뒤에는 사진 Blob, mission, 업로드 경로, 저널 초안과 동기화 상태를 날짜별 IndexedDB에 먼저 저장한다.
- 확정 사진은 기존 `posts` upsert와 `grid_images`/`client_meta.gridImages` fallback으로 저장한다. 업로드 성공 뒤 Post 저장이 실패하면 업로드 경로를 보존해 재시도에서 중복 업로드하지 않는다.
- 동일 사용자·`local_date`의 로컬 초안과 서버 Post는 하나의 논리적 기록으로 병합하며, 동기화 대기 중 최신 로컬 상태가 표시 우선권을 가진다.
- 8장 완성은 서버 저장 전에도 병합 결과에서 완료 페이지·배지에 반영되며, 서버 성공 후 같은 날짜는 중복 계산하지 않는다.
- 기록 읽기는 Post별·이미지별 요청을 만들지 않고, 고유 Storage 경로를 한 번의 `createSignedUrls` 호출로 서명해 3×3과 대표 이미지에 재사용한다. 430×932 QA에서 개별 서명 요청 누적과 504로 복구가 지연된 사실을 확인해 적용한 최소 수정이다.
- 새 Supabase migration은 추가하지 않았다.
...
- 현재 웹 전달 계층은 Vercel 정적 CDN이고 앱 데이터 요청은 브라우저/Capacitor 클라이언트가 Supabase에 직접 보낸다. 별도의 항상 실행되는 Hueday API 서버는 없다.
- 이 구조는 서버 프로세스 운영 부담 없이 초기 베타에 적합하지만, 아직 동시 사용자 부하 테스트로 특정 수용량을 입증하지 않았다. 초대 규모가 정해지면 예상 peak의 2배에서 핵심 읽기·저장 경로를 검증하고 실제 결과만 용량 근거로 기록한다.
