# ColorWalk Play Store Internal Testing

Use this when you want friends to install ColorWalk through Google Play instead of sideloading an APK.

## What You Need

- A Google Play Console developer account.
- The ColorWalk app created in Play Console.
...
1. Open Play Console.
2. Create or open the ColorWalk app.
...
```text
ColorWalk friend beta: daily color mission, camera color capture, journal, history, and story export.
```
...
```text
오늘의 색을 찾아 기록하고, 9:16 스토리 이미지로 공유하는 데일리 컬러 산책.
```

Permission explanation:

```text
ColorWalk uses camera access to sample real-world colors and location access only to choose a weather/time-based daily color mission.
```

Privacy note:

```text
ColorWalk stores each tester's entries under their Supabase user ID. Entries are private and protected by owner-scoped database and storage policies.
```
