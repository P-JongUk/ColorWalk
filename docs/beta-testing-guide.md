...
PWA 카메라와 설치 동작은 HTTPS에서만 안정적으로 검증됩니다. 셔터는 기본적으로 앱 안에서 바로 촬영하고, 지원되는 Android Chrome 계열 브라우저에서는 줌 슬라이더가 표시됩니다. 브라우저가 줌을 노출하지 않으면 인앱 촬영은 유지되고, 필요할 때만 작은 `기본 카메라` fallback 버튼을 사용합니다.
...
- 카메라 권한 허용
- 오늘의 색 주변에서 최대 8컷 수집
- 지원 기기에서 인앱 줌 슬라이더/1x·2x·3x 버튼 확인
- 앨범 선택 fallback 확인
- 저널에서 컬러 이름, 무드 문장 입력
- 같은 날짜 재저장 시 교체 확인
