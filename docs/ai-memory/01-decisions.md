...
- 현재 계획의 시간축은 3개월 로드맵이 아니라 외부 베타 출시까지의 critical path입니다.
- 출시 후 검증 가능한 성장, 수익화, 공개 피드, 대형 소셜 기능은 출시 차단 요소에서 제외합니다.
- 속도를 이유로 인증/RLS, 개인정보 보호, 데이터 손실 방지, 필수 빌드 검증과 실제 기기 핵심 여정 QA를 생략하지 않습니다.
- 계획에는 출시 채널, 반드시 고칠 것, 미룰 것, 의존 순서, 자동 검증과 사용자 수동 작업을 분리해 기록합니다.
