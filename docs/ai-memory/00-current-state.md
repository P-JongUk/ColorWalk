...
> **현재 실행 순서:** M2-3 Android/PWA 인플레이스 QA는 M7 출시 gate로 유지한다. M3 Living Hue Deck과 M4 선택형 일상 미션 팩은 모두 `main`에 통합 완료됐으며, 다음 구현은 M5 Hueprint/Color DNA·Color Capsule → M6 통합 디자인·접근성·성능 → M7 출시 검증 → 버전 1 출시다. Hue Canvas는 출시 후 필수 초기 업데이트, Hue Drop은 별도 첫 소셜 업데이트다.
