...
- [ ] PWA, Android, 스토어 출시 경로를 구분하고 이번 완성 출시의 정확한 완료 지점을 확정하기
- [ ] Color Hunt, 측정, 안전한 Color Relay, Hueprint/Color Capsule, 실제 배지 보상을 `출시 전 필수 / 병렬 가능 / 난도 때문에 후순위`로 분류하고 완성 출시 범위를 확정하기
- [ ] 1컷 기록과 8컷 완성의 모순을 가장 작은 카피·동작 변경으로 정렬하기
- [ ] 성공 요소를 의존 관계 순서로 구현하고 기능별 완료 조건을 통과시키기
- [ ] `docs/release-readiness.md`의 전체 검증을 현재 출시 후보 커밋에서 다시 실행하기
- [ ] 실제 휴대폰에서 가입 → 촬영 → 저장 → 공유·Relay·Hueprint 핵심 여정을 확인하고 통과 즉시 출시하기
- [ ] 다음 대형 기능을 `feature/<기능명>` 브랜치에서 시작
- [ ] 첫 기능 작업 전 `graphify query`로 관련 구조 확인
- [ ] 작업 완료 후 세션 기록을 남기고 검증 결과를 커밋 메시지와 함께 저장
- [ ] 베타 사용자에게 1컷 기록과 8컷 완성 중 부담·완성감이 좋은 규칙을 검증하고 후속 카피/버튼/보상을 조정하기
