...
> - **현재 체크포인트:** CP6 live `product_events` 적용 사실·문서·main 병합 준비. `grid_images` 전환·backfill·fallback 종료와 다른 M2 브랜치는 범위 밖이다.
...
> - **다음 한 가지 작업:** CP6 문서 변경을 검증·push한 뒤 feature 전체 diff를 검토해 main에 병합한다. 다음 M2 구현은 local master·offline sync 범위를 별도 승인받은 후 시작한다.
