...
> - **현재 체크포인트:** CP3 완료 대기 — 핵심 퍼널 이벤트 연결. 다음 CP4는 설치된 Playwright CLI만 쓰는 local 430×932 최소 E2E이며 예상 수정은 이 인계 블록, `scripts/`의 E2E runner, 필요한 안정 locator와 테스트 이미지 fixture다. `grid_images` 전환·backfill·fallback 종료는 범위 밖이다.
...
> - **수정 중이거나 dirty인 파일:** `docs/ai-memory/02-next-tasks.md`, `src/App.tsx`, `src/components/AuthGate.tsx`, `src/components/JournalView.tsx`, `src/components/StoryStudio.tsx`
> - **마지막 통과 검증:** `npm test -- --run` (21 tests), `npm run lint`, `npm run build`; `npm run verify:supabase` (기존 경로 통과, `productEvents.migration_pending`은 적용 전 기대 결과).
> - **실패한 검증과 이유:** `npx supabase migration list --linked`는 local project link가 없어 사용할 수 없었고, Supabase CLI `projects list`는 `SUPABASE_ACCESS_TOKEN`이 없어 관리자 연결을 만들 수 없었다. publishable-key live verify로 새 테이블 부재를 확인했다.
> - **다음 한 가지 작업:** `git diff --check` 후 CP3를 한국어 커밋·원격 push하고, Playwright skill 지침과 현재 CLI 사용 경로를 읽는다.
