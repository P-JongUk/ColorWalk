...
#### 구현 상태 보정 — 2026-07-23 KST

`feature/color-hunt-contract`의 `e6036c5`와 `f135aae`가 아래 항목을 구현했고 lint·unit test·production build·라이브 Supabase 검증·Capacitor sync·Android debug/release build를 통과했다. 이 보정이 같은 의미의 기존 미체크 항목을 대체한다.
...
- [ ] 실제 브라우저 430×932 및 Android 카메라·강제 종료 복구·날짜 전환 QA, 테스트 데이터 재시드는 최종 체크포인트

- [ ] 현재 홈·카메라·저널·저장·히스토리의 1장/8장 동작 추적
- [x] 1장 유효 기록과 8장 완성의 의미 사용자 승인
- [x] 현지 날짜마다 새 색 선택, 첫 사진 확정 후 잠금, 자정 종료 사용자 승인
- [ ] 중앙 미션 색 + 주변 8장 규칙을 모든 카피에 일치
- [ ] 사진 색 추출과 매칭률 UI·스토리·히스토리 잔재 제거
- [ ] legacy `match_rate`는 DB 호환과 제거 migration 계획을 분리
- [ ] 첫 추천 + 3회 맥락 추천 뒤 전체 색 균등 랜덤과 색 잠금 구현
- [ ] 1~7장 일일 기록의 안전한 저장·같은 날 복귀·현지 자정 종료 구현
