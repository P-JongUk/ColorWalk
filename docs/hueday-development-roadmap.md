...
> **2026-07-29 실행 순서 정정 — 이 블록이 아래의 이전 출시 순서보다 우선한다.** `M2-3 수동 master 정리·업데이트 안전` → `M3 Living Hue Deck` → `M4 최소 미션 팩` → `M5 Hueprint·Color Capsule` → `M6 통합 디자인·접근성·성능` → `M6.5 실제 화면·제품 카피 마감` → `M7 실기기·보안·출시 검증` → 무료 버전 1 출시 → 실제 인플레이스 업데이트 보존 확인 → `M8M 결제·entitlement·Hueday Studio` → `M8A Hue Canvas 필수 초기 업데이트` → `M8B Hue Drop 첫 소셜 업데이트` → 사용 데이터 기반 후보 재검토. 결제 안전 계약: `docs/post-launch-monetization-and-payment-safety.md`.
...
| M5 | Hueprint·Color Capsule | 주간 회고와 Story 공유를 Deck 원본 기록 위에서 연결한다. 완성 강제·연속 출석·랜덤 보상을 넣지 않는다. |
| M6 | 통합 디자인·접근성·성능 | 430×932의 일반 사용자 경로와 기본 Android/PWA 성능을 다듬는다. 고위험/비현실 조합은 P2 보류다. |
| M6.5 | 실제 화면·제품 카피 마감 | 승인된 Modern Warm Archive를 실제 브라우저에서 확인하고, 재추천 6회·Story 중앙 표기 선택·자연스러운 색 이름을 좁게 구현·검증한다. 디자인 방향을 다시 열지 않는다. |
...
### M5 Hueprint·Color Capsule 완료 조건

- [x] 주간 Hueprint가 canonical `mission_hex`가 유효한 기록만으로 파생되고, 무효 mission_hex 기록은 History/Deck/Story는 유지한 채 Hueprint/Capsule 파생에서만 제외된다.
- [x] 월간 Color Capsule이 1~7장 부분 기록과 8장 완성 기록을 모두 포함하고, 무효 색만 있는 월은 tile을 만들지 않는다.
- [x] 로컬 표지 preference가 owner/week로 격리되고 stale 참조는 기본 표지로 안전하게 복구된다.
- [x] `다시 만난 색`이 동일 월·일 최근 연도, 이전 달 ±3일 순으로 선택되고 후보가 없으면 카드를 숨긴다.
- [x] 전용 9:16 export가 기존 Story Studio의 html2canvas/download/Web Share/Capacitor Share 패턴을 `shareImage.ts` 공용 helper로 재사용하고, 워터마크 없는 1080×1920 PNG를 만든다.
- [x] `hueprint_exported`/`color_capsule_exported`/`*_share_opened`가 실제 전달 성공 뒤에만 기록되고, 실패·취소는 완료 이벤트를 남기지 않는다(430×932 Playwright QA로 확인, 실패 재현 포함).
- [x] tier 0~4가 `getHueprintDetailTier()` 결과 하나만 반영하고 장식 class가 누적되지 않는다.
- [x] 기존 Story UI/template/sticker/callback과 `getUnlockedBadges()` 결과에 회귀가 없다.
- [x] 새 DB migration, 서버 렌더링, 결제 UI, Hue Canvas/Hue Drop을 추가하지 않았음을 diff로 확인한다.
...
- [ ] 카메라·앨범 대표 사진의 cloud preview가 EXIF 위치를 포함하지 않으며 raw/local master가 업로드되지 않는지 확인한다.
- [ ] 위치 권한은 foreground 추천에서만 요청되고 거부 fallback이 동작하며, 좌표·장소명이 Post/client_meta/analytics/log에 남지 않는지 확인한다.
- [ ] 날씨 제공자 요청은 기능에 충분한 coarse 좌표로 줄이고, 정확 좌표를 보관·재사용하지 않는다.
- [ ] private bucket, owner RLS, 짧은 signed URL, anonymous/cross-user denial을 실제 출시 환경에서 재검증한다.

### M6.5 실제 화면·제품 카피 마감 완료 조건

- [ ] 430×932 실제 브라우저에서 Home 0장/부분/완료, Journal, Story preview/export를 확인하고 승인된 시안과 다른 시각 결함만 수정한다. 360px·200% 확대도 핵심 CTA와 3×3 정사각형을 확인한다.
- [ ] 최초 문맥 추천 뒤 `다른 색`은 최대 6회이며, 1~3회 문맥 추천과 4~6회 전체 catalog 균등 무작위가 당일 이미 본 모든 색을 제외한다.
- [ ] Story의 `중앙 색 이름 표시`는 기본 켜짐이며, 끄면 중앙 이름·HEX만 preview/export에서 숨고 원본 기록과 다른 화면은 바뀌지 않는다.
- [ ] 한국어·영어 mission catalog 이름을 전수 검토해 감성적이되 자연스럽고 의미가 통하는 이름만 남긴다. ID·HEX와 기존 기록은 backfill하지 않는다.
- [ ] 디자인 방향, DB schema, 새 서버/package, 보상 경제는 변경하지 않고 변경된 happy path와 현실적인 복구 path만 검증한다.
...
- 현재 작업: **M6.5 실제 화면·제품 카피 마감 준비.** M7 store asset과 최종 QA 전에 실제 브라우저에서 승인 디자인을 보고 재추천·Story 표기·색 이름을 좁게 마감한다.
- 다음 한 작업: **Terra high 단일 작업에서 실제 430×932 브라우저 화면을 확인하고 M6.5 계약만 구현·검증한다.** 완료 뒤 M7 Android 인플레이스 업데이트, 보안, 한국어·영어 store 자료와 Play 정책을 실행한다.
...
- 첫 색은 날씨·시간·선택적 대략 위치에 맞춰 추천한다. 승인된 M6.5 변경은 `다른 색`을 최대 6회로 제한하고, 1~3회는 문맥 추천, 4~6회는 전체 색 균등 무작위로 하되 당일 이미 본 모든 색을 제외한다. 현재 M1 구현은 현재 색 제외·횟수 무제한이므로 M6.5에서 차이를 닫는다.
...
- `npm test -- --run`
- `npm run build`
- 브라우저 최초 추천·교체 1~3회 문맥·4~6회 균등 랜덤·당일 노출 색 제외·6회 종료·첫 사진 잠금·자정 전환·1~7장 기록·8장 완성 QA
- Android 촬영·복구 QA
...
> 이 절의 Canvas 구현 목록은 현재 첫 출시 작업에 적용하지 않는다. 현재 M5는 Living Hue Deck 원본 기록 위의 Hueprint·Color Capsule이며, Canvas production은 M8A다.
