...
1. 앱이 날씨/시간/선택적 대략 위치에 맞는 색을 추천하고, `다른 색`은 최대 6회 제공한다. 1~3회는 같은 문맥, 4~6회는 전체 큐레이션 균등 무작위이며 그날 이미 본 색은 다시 보여주지 않는다.
...
- daily mission은 첫 추천과 1~3회 교체까지 날씨/시간/선택적 대략 위치 문맥을 유지한다. 4~6회 교체는 전체 큐레이션 색을 같은 확률로 뽑고, 해당 날짜에 이미 노출한 모든 색을 제외한다. 여섯 번째 뒤에는 추가 교체를 제공하지 않는다.
...
1. Color Hunt contract:
   - 현지 날짜별 새 색, 문맥 추천 1회와 `다른 색` 최대 6회(1~3회 문맥, 4~6회 전체 색 균등 무작위), 당일 노출 색 전체 제외, 첫 사진 잠금, 1–7장 일일 기록, 8장 완료 규칙을 코드·카피에 일치.
   - 사진 색 추출·매칭률 UI 제거.
