...
> **최신 보상 우선순위(2026-07-26):** 첫 출시의 주 보상은 완성 3×3이 자동으로 만드는 Living Hue Deck 봉인 카드와 Color Volume 누적이다. 1/3/5장은 가벼운 시각 성장, 8장은 주요 완성 단위이며 어느 단계도 streak·벌점·소모 재화가 아니다. Hue Canvas 재료/사용량 보상은 보류하고, Hue Drop 참여 보상도 출시 후 별도 검증 전에는 만들지 않는다.
...
- 사용자 기기의 현지 날짜마다 새 미션 색을 선택한다. 첫 추천과 3회의 맥락 추천 뒤에는 전체 색 목록에서 균등 랜덤으로 고를 수 있다.
