...
2. Use `docs/development-reference-guide.md` to select the feature-specific source documents. Living Hue Deck work must include `docs/living-hue-deck-product-spec.md` and `docs/launch-scope-and-update-safety-contract.md`; Hue Canvas is an approved mandatory early post-launch update and must include `docs/hue-canvas-product-spec.md`, storage strategy, and release readiness; Hue Drop is the first social update and must include `docs/hue-drop-post-launch-spec.md`. Storage work must include `docs/data-storage-sync-and-cost-strategy.md`, and design work must include `docs/design-reference-index.md`. For explicitly reopened Hue Room work, include its historical spec and roadmap and first verify new user approval.
...
- When the user immediately relays a completed task's verification report, reuse it rather than mechanically rerunning the full suite: confirm the verified commit is still `HEAD`, there are no code changes after it, and the remote branch matches. Then run only `git diff --check`, Git/remote checks, and any new narrow check needed for the current task. Rerun affected verification only if code, dependencies, generated native inputs, schema/policy, environment-sensitive configuration, or a release gate changed after the reported result.
...
- post-launch Hue Canvas or Hue Drop: `docs/hue-canvas-product-spec.md` or `docs/hue-drop-post-launch-spec.md`, plus storage/release source documents; keep Canvas as mandatory early feature update and Hue Drop as first social update
